<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center space-x-3">
            ...
            <div>
                <h2 class="font-bold text-2xl text-white tracking-wide">Edit Booking</h2>
                <p class="text-kemnaker-200 text-sm mt-0.5">{{ $booking->nama_pemesan }}</p>
            </div>
        </div>
    </x-slot>
            ...
            <form method="POST" action="{{ route('pemesanan.update', $booking->id) }}" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input id="nama_pemesan" name="nama_pemesan" type="text" value="{{ old('nama_pemesan', $booking->nama_pemesan) }}" required
                            class="block w-full rounded-lg border-slate-300 focus:border-kemnaker-500 focus:ring-kemnaker-500 shadow-sm text-slate-800">
                    ...
                    <div>
                        ...
                        <input id="nomor_hp" name="nomor_hp" type="text" value="{{ old('nomor_hp', $booking->nomor_hp) }}" required
                            class="block w-full rounded-lg border-slate-300 focus:border-kemnaker-500 focus:ring-kemnaker-500 shadow-sm text-slate-800">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input id="checkin_date" name="checkin_date" type="date"
                            value="{{ old('checkin_date', $booking->checkin_date->format('Y-m-d')) }}" required
                            class="block w-full rounded-lg border-slate-300 focus:border-kemnaker-500 focus:ring-kemnaker-500 shadow-sm text-slate-800">
                    ...
                    <div>
                        ...
                        <input id="checkout_date" name="checkout_date" type="date"
                            value="{{ old('checkout_date', $booking->checkout_date->format('Y-m-d')) }}" required
                            class="block w-full rounded-lg border-slate-300 focus:border-kemnaker-500 focus:ring-kemnaker-500 shadow-sm text-slate-800">
                ...
                <div>
                    ...
                    <input id="jumlah_kamar" name="jumlah_kamar" type="number" min="1" value="{{ old('jumlah_kamar', $booking->jumlah_kamar ?? 1) }}" required
                ...
                <div>
                    ...
                    <textarea id="catatan" name="catatan" rows="3"
                        class="block w-full rounded-lg border-slate-300 focus:border-kemnaker-500 focus:ring-kemnaker-500 shadow-sm text-slate-800">{{ old('catatan', $booking->catatan) }}</textarea>
