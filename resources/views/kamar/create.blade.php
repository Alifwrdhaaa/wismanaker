<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center space-x-3">
            ...
            <div>
                <h2 class="font-bold text-2xl text-white tracking-wide">Tambah Kamar / Ruangan</h2>
                <p class="text-kemnaker-200 text-sm mt-0.5">Isi data kamar baru</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="max-w-2xl">
        <div class="admin-card rounded-xl p-8">
            <form method="POST" action="{{ route('kamar.store') }}" enctype="multipart/form-data" class="space-y-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input id="harga" name="harga" type="number" value="{{ old('harga', 0) }}" required min="0"
                            class="block w-full rounded-lg border-slate-300 focus:border-kemnaker-500 focus:ring-kemnaker-500 shadow-sm text-slate-800">
                    ...
                    <div>
                        ...
                        <input id="jumlah_unit" name="jumlah_unit" type="number" value="{{ old('jumlah_unit', 1) }}" required min="1"
                            class="block w-full rounded-lg border-slate-300 focus:border-kemnaker-500 focus:ring-kemnaker-500 shadow-sm text-slate-800">
                ...
                <div>
                    ...
                    <textarea id="deskripsi" name="deskripsi" rows="4"
                        class="block w-full rounded-lg border-slate-300 focus:border-kemnaker-500 focus:ring-kemnaker-500 shadow-sm text-slate-800">{{ old('deskripsi') }}</textarea>
                    @error('deskripsi') <p class="mt-1.5 text-sm text-red-600">{{ $message }}</p> @enderror
                </div>

                <div class="flex items-center gap-4 pt-2">
                    <button type="submit" class="btn-kemnaker rounded-lg px-6 py-2.5">Simpan Kamar</button>
